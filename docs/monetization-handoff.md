...
- `docs/organization/article-automation-workflow.md`
...
- PR目的のSNS投稿、X投稿、Instagram Reels投稿、週次SNS企画、投稿キュー投入は停止する。
- 記事制作は `docs/organization/article-automation-workflow.md` を正とする。
- 記事制作は2本軸で運用する。
  - 月曜 3:00 JST: AIアップデート記事。最新のAI/開発者ツール更新を日本語で紹介し、日本での活用方法へ翻訳する。
  - 木曜 3:00 JST: 海外Micro-SaaS / AI SaaS / Exit / GTM事例記事。SaaS Case DBの資産にもなる記事を作る。
- 新規記事ファイルは `src/content/posts/[slug].md` に作成する。
- slugは英語小文字、数字、ハイフンのみ。日付だけのファイル名、日本語、アンダースコアは使わない。
- Pattern A記事では、記事作成と同時にSaaS Case DBの行を追加または更新する。
- AIアップデート記事では、原則としてSaaS Case DB行は追加しない。具体的なSaaS事例を扱う場合だけ追加する。
...
- 月曜にAIアップデート記事を1本追加する。
- 木曜に海外Micro-SaaS / AI SaaS事例記事を1本追加する。
- Pattern AならDB行も追加または更新する。AIアップデート記事では原則DB行を追加しない。
- Pro先行案内登録者数を確認する。
...
1. Codex automation で週2本の記事制作を進める。
2. 月曜はAIアップデート、木曜は海外Micro-SaaS / AI SaaS事例にする。
3. Pattern Aなら記事とSaaS Case DB行をセットで更新する。AIアップデート記事では原則DB行を追加しない。
4. `npm run articles:analyze` と `npm.cmd run build` を通す。
5. commit / push 後に公開URLを確認し、Search Console indexing が必要なURLを記録する。
